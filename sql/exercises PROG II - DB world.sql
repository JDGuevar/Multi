...
create function lenguas(code char(3)) returns text
deterministic
begin
declare final boolean default false;
declare resultado text default '';
declare lengua char(30);
declare myCursor cursor for select language from countrylanguage where countrycode = code;
declare continue handler for not found set final = true;

open myCursor;
	bucle:loop
		fetch myCursor into lengua;
        if final then 
        leave bucle;
        end if;
        set resultado = concat(resultado, lengua, ', ');
	end loop bucle;
close myCursor;
...
drop function lenguas$$


select lenguas('ESP') $$ -- Basque, Catalan, Galecian, Spanish
select lenguas('AGO') $$ -- Ambo, Chokwe, Kongo, Luchazi, Luimbe-nganguela, Luvale, Mbundu, Nyaneka-nkhumbi, Ovimbundu
...
SELECT group_concat(language) FROM countrylanguage WHERE CountryCode = 'ESP' group by countrycode$$ -- Basque, Catalan, Galecian, Spanish
SELECT language FROM countrylanguage WHERE CountryCode = 'AGO'$$
...
create procedure todasLasLenguas()
begin

        select Name, lenguas(Code)
        from country;
        
end $$


select concat(Name,' (',lenguas(code), ')') as llista from country$$


drop procedure todasLasLenguas$$




CALL todasLasLenguas $$
/*
